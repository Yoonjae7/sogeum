...
function normalizeBuses(data,routeId,receivedAt){
 ...
 return (data.result.realTime.list||[]).flatMap(row=>{
  ...
  return xs.flatMap((x,i)=>{const coord=unproject(x,ys[i]);if(!coord.every(Number.isFinite)||coord[0]<bounds[0]||coord[0]>bounds[2]||coord[1]<bounds[1]||coord[1]>bounds[3])return[];
...
module.exports={unproject,normalizeBuses,normalizeStop};
