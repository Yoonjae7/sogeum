export function setupExplore({catalog,neighborhood,focus,toggleStops,closeTransit}){
 ...
 const places=[{name:'철산역',coord:[126.8675973,37.4760012],kind:'place',description:'7호선 · 철산동'}, {...neighborhood.church,kind:'place',description:'하안로 437 · 병원 남동쪽'}, {name:'광명성애병원',coord:[126.871875,37.4734763],kind:'place',description:'디지털로 36'},...catalog.stops.map(s=>({...s,kind:'stop',description:`정류소 ${s.ref||'번호 미등록'} · ${s.source==='GBIS'?'공식 좌표':'OSM 좌표'}`}))];
 ...
 function close(){generation++;request?.abort();request=null;panel.hidden=true;clearInterval(timer);timer=null;selected=null;stopData=null;vehicles=[];document.body.classList.remove('explore-focus');}
 ...
 function openRoute(route){close();selected={type:'route',value:route};base(`${route.name}번 버스`,`${route.region||'철산'} · ${route.operator||'경기버스정보'}`);refresh();timer=setInterval(refresh,30000);}
 function renderStops(){close();base('버스 · 정류소',`${catalog.stops.length}곳 · 철산`);const content=$('#explore-content');const note=document.createElement('p');note.textContent='버스는 지도에 자동 표시됩니다. 노선을 선택하면 차량 목록과 위치를 볼 수 있어요.';content.append(note);const routes=document.createElement('div');routes.className='route-grid';catalog.routes.forEach(r=>routes.append(button(r.name,()=>openRoute(r),'route-chip')));content.append(routes);const n=document.createElement('p');n.className='data-note';n.textContent='정류소 전체 목록은 검색에서 찾을 수 있어요. GBIS와 OSM에 등록된 정류소를 표시하며, 신규·임시 정류소는 누락될 수 있습니다.';content.append(n);}
 async function refresh(){
  ...
  try{
   ...
   if(current.type==='route'){
    vehicles=data.buses||[];const count=document.createElement('strong');count.className='bus-count';count.textContent=vehicles.length?`지도 안 운행 차량 ${vehicles.length}대`:'지도 안에서 수신된 차량이 없어요';content.append(count);
    vehicles.forEach((bus,i)=>content.append(button(`${current.value.name} · 차량 ${i+1} 위치 보기`,()=>focus(bus.coord,'bus'),'vehicle-row')));
    const n=document.createElement('p');n.className='data-note';n.textContent='차량 번호를 지도에 표시하며, 수신된 GPS 위치 사이를 도로를 따라 보간합니다. 실제 위치와 지연이 있을 수 있어요.';content.append(n);
   }else{
    stopData=data;
  ...
  }catch{if(token===generation){vehicles=[];content.replaceChildren();const p=document.createElement('p');p.textContent='현재 버스 정보를 불러올 수 없어요. 잠시 후 다시 시도해 주세요.';const a=document.createElement('a');a.href='https://www.gbis.go.kr/gbis2014/schBus.action';a.target='_blank';a.rel='noreferrer';a.textContent='경기버스정보에서 확인 ↗';content.append(p,button('다시 확인',refresh,'route-chip'),a);}}
 ...
 $('#close-explore').onclick=close;$('#focus-church').onclick=()=>openPlace(neighborhood.church);$('#bus-layer').onclick=renderStops;
